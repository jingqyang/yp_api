...
class ShoppingCart extends Model{
    ...
    public function getSelectedCartInfo($wx_id){
        ...
        $pro_res = Db::table('store_shopping_carts')->alias('carts')->field('carts.id as cart_id,con.id as pro_id,pro.p_name,con.con_content1,con_content2,con.price as associator_price,carts.count,carts.selected,img.thumb')
            ->join('store_pro_config con','carts.pro_id = con.id','LEFT')
            ->join('store_products pro','con.pro_id = pro.id','LEFT')
            ->join('store_pro_images img','img.p_id = pro.id','LEFT')
            ->where($where)
        ...
        $total_amount = Db::table('store_shopping_carts')->alias('carts')->field('carts.id as cart_id,pro.p_name,con.con_content1,con_content2,con.price as associator_price,carts.count,carts.selected,img.thumb')
                    ->join('store_pro_config con','carts.pro_id = con.id','LEFT')
                    ->join('store_products pro','con.pro_id = pro.id','LEFT')
                    ->join('store_pro_images img','img.p_id = pro.id','LEFT')
                    ->where($where)
