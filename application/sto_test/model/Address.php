...
class Address extends Model{
    ...
    public function getAddressInfo($openid){
        $res = DB::table('store_address')->field('id as a_id,rec_name,mobile,province,city,area,address,is_default')->where("mem_id = '$openid' and status = 1 ")->order('create_time desc')->select();
        ...
        foreach($res as $val){
            ...
            $result[$index]['is_default'] = $val['is_default'];
    ...
    public function addAddress($param){
        ...
        $is_default = !empty($param['switch']) ? $param['switch'] : false;
    ...
    public function getAddressInfoById($a_id){
        $res = DB::table('store_address')->field('id as a_id,rec_name,mobile,province,city,area,address,is_default')->where("id = $a_id")->find();
